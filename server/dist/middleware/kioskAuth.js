...
if (!KIOSK_SECRET) {
    throw new Error("[kioskAuth] KIOSK_SECRET is not set in environment variables. " +
        "Generate one with: node -e \"console.log(require('crypto').randomBytes(32).toString('hex'))\"");
}
if (KIOSK_SECRET.length < MIN_SECRET_LENGTH) {
    throw new Error(`[kioskAuth] KIOSK_SECRET is too short (${KIOSK_SECRET.length} chars). ` +

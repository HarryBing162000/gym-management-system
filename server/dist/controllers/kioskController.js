...
exports.kioskWalkInCheckOut = exports.kioskWalkInLookup = exports.kioskMemberCheckOut = exports.kioskMemberCheckIn = exports.kioskSearch = void 0;
...
const sanitizeSearchQuery = (q) => q.replace(/[.*+?^${}()|[\]\\]/g, "").trim();
...
const kioskSearch = async (req, res) => {
    try {
        ...
        if (/^GYM-\d+$/i.test(q)) {
            // Exact GYM-ID match
            members = await Member_1.default.find({ gymId: q.toUpperCase() }).select(MEMBER_PROJECTION);
        }
        else if (/^WALK-?\d*/i.test(q)) {
            // Partial WALK-ID match — search today's walk-ins
            walkIns = await WalkIn_1.default.find({
            ...
            [members, walkIns] = await Promise.all([
                Member_1.default.find({ name: { $regex: q, $options: "i" } })
                    .select(MEMBER_PROJECTION)
                    .limit(5),
                WalkIn_1.default.find({ name: { $regex: q, $options: "i" }, date: today })
                    .select("walkId name passType checkIn isCheckedOut date")
                    .limit(3),
            ]);
        }
...
exports.kioskMemberCheckOut = kioskMemberCheckOut;
...
const kioskWalkInLookup = async (req, res) => {
    try {
...
exports.kioskWalkInLookup = kioskWalkInLookup;
...
const kioskWalkInCheckOut = async (req, res) => {
    try {
...
exports.kioskWalkInCheckOut = kioskWalkInCheckOut;
